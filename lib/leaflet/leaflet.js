...
//		apriLeafLetBase = new ApriCore.ApriLeafLetBase({viewCoordinates: [52.490, 4.97], viewZoomLevel: 12, mapHeaderHeight: 0, mapFooterHeight:0 });
		apriLeafLetBase = new ApriCore.ApriLeafLetBase({viewCoordinates: appConfig.viewCoordinates, viewZoomLevel: appConfig.viewZoomLevel, mapHeaderHeight:appConfig.mapHeaderHeight, mapFooterHeight:appConfig.mapFooterHeight });
				...
				if (e.layer.options) {
					...
					if (e.layer.options.legendPng) {
						if (e.layer.layerControl == null) {
							...
							e.layer.layerControl = initLayerCtrl({type: "infoCtrl", url: e.layer.options.infoCtrl} );
						}		
					}
...
*/
        ...
        function onEachAireasGridGemFeature (feature, layer) {
            ...
            //if (feature.properties && feature.properties.bu_naam) {
                layer.bindPopup(feature.properties._dateFormatted + 
					'<BR/>'+feature.properties.avg_type + ": " + feature.properties.avg_avg
					);
            //}
...
*/						
    			...
    			//	if (map) {
       					layerPopup = L.popup({'offset': L.point(5,-80)})
